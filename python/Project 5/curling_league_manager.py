    ...
    def save(self, file_name):   # <----- check for correctness
        if not file_exists(file_name):
            with open(file_name, mode="wb") as f:
                pickle.dump(self.leagues, f)   #  <---- self.leagues???
        else:
            my_string = file_name + ".backup"
    ...
    def import_league_teams(self, league, file_name):
        with open(file_name, "rt", encoding="UTF8") as f:
            try:
                ...
                index = 0
                while index < a:
                    if rows[index][0] not in teams:
                        teams.append(rows[index][0])
                        team = Team(self.next_oid(), rows[index][0])
                        league.add_team(team)
                        print(team)
                        while rows[index][0] == team.name:
                            if rows[index][1] not in members:
                                members.append(rows[index][1])
                                member = TeamMember(self.next_oid(), rows[index][1], rows[index][2])
                ...
                print(e)
                print("Error loading league. One of your parameters is incorrect.")



    def export_league_teams(self, league, file_name):
        with open(file_name, "at", encoding="UTF8") as f:
            try:
                writer = csv.writer(f)
